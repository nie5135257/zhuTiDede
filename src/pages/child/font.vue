<template>
	<div id="font">
		...
		<el-table :data='font' :stripe='true' style="width: 100%" height='7.54rem'>
				...
				<el-form-item label="上传图片" prop="src">
					<el-upload 
						...
						<div slot="tip" class="el-upload__tip">
							请上传2张图片 （只能上传jpg/png文件，且不超过500kb,文件名相同会被替换）
						</div>
					</el-upload>
				</el-form-item>
		...
		methods: {
			...
			handleDelete(index, row) {
				...
				this.$confirm('此操作将删除该用户, 是否继续?', '提示', {
					...
					console.log(row._id)
					this.$http.delete('/font?_id=' + row._id).then(res => {
						this.$message({
							type: 'success',
							message: '删除成功!'
						});

						if(this.font.length === 1 && !this.souSuo) {
				...
				this.$refs[formName].validate(async(valid) => {
					if(valid) {
						...
						if(this.ruleForm.src.length !== 2){
							...
							this.$http.put('/font',{_id:this._id,data: this.ruleForm}).then(res=>{
								this.$message({
								type: 'success',
								message: res.data
							});
